...
The runner requires compiler 0.26.0 on PATH (on Windows, inside WSL), compiles the source afresh with `--skip-zk` into ignored `.compact-generated/`, and executes the emitted contract using pinned Compact runtime 0.9.0 / onchain-runtime 0.3.0. It does not substitute a TypeScript lifecycle model. Twelve local runtime tests cover the successful lifecycle, wrong-secret/public-commitment attacks, deadline and state boundaries, and rejected bids. Opening's emitted transcript also succeeds at its original time and fails replay at/after the deadline using the actual query runtime. These are simulated query contexts, not cryptographic proof or live-network tests.
