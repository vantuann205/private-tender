...
async function main() {
  ...
  if (new URL(connectionString).hostname.includes("-pooler"))
    throw new Error("Use a direct database URL for migrations.");
  const pool = new Pool({
    connectionString,
    max: 1,
...
main().catch(() => {
  console.error(
    "Migration failed. Check the direct database URL and database permissions; no credentials were logged.",
  );
  process.exitCode = 1;
});
