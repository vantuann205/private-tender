...
export function submitDemoBid(
  ...
  if (tenderStatus(tender, now) !== "Open")
    throw new Error("Bidding is not open for this tender.");
  if (
    !proof?.eligible ||
    proof.tenderId !== tender.id ||
    proof.mode !== "development"
  )
    throw new Error("Check eligibility for this tender before bidding.");
  if (!/^\d{1,11}(\.\d{1,2})?$/.test(amount) || Number(amount) <= 0)
    throw new Error(
      "Enter a positive amount below 100 billion USD, with at most two decimal places.",
    );
