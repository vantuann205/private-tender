...
export type Tender = Omit<TenderInput, "requirements"> & {
  id: string;
  requirements: string[];
  organization: string;
  category: string;
...
export function validateTender(
  ...
  for (const key of ["title", "description", "requirements"] as const) {
    ...
    if (!input[key].trim() || input[key].length > max)
      errors[key] = `Enter ${key} (1–${max} characters).`;
  }
  if (
    !Number.isFinite(Date.parse(input.deadline)) ||
    Date.parse(input.deadline) <= now
  )
    errors.deadline = "Choose a future bidding deadline.";
  if (!WINNER_RULES.some((rule) => rule === input.winnerRule))
    errors.winnerRule = "Choose a supported winner rule.";
  if (!["Draft", "Open"].includes(input.status))
    errors.status = "New tenders must be Draft or Open.";
  return errors;
}
export function tenderStatus(
  ...
  return tender.status === "Open" && Date.parse(tender.deadline) <= now
    ? "Closed"
    : tender.status;
}
