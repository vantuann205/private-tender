...
export function assertSameOrigin(request: Request) {
  if (request.headers.get("origin") !== new URL(request.url).origin)
    throw new RequestError("Same-origin request required.", 403);
}
export function parseTenderInput(value: unknown): TenderInput {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new RequestError("Invalid tender fields.");
  ...
  if (
    ...
    throw new RequestError(
      "Only the supported public tender fields are accepted.",
    );
  ...
  if (
    ...
    throw new RequestError("Use an ISO deadline with a time zone.");
...
async function readJsonBody(request: Request): Promise<unknown> {
  if (
    ...
    throw new RequestError("JSON content type required.", 415);
  const limit = 32_768;
